...
namespace QTetris
{
    public class FallItemShortL : FallItem
    {

        public FallItemShortL(Color color, int pos_x, int pos_y, byte actualState)
            : base(color, pos_x, pos_y, actualState )
        {
            byte stateWindowMax = 2;

            this.states[0] = new FallItemState(stateWindowMax);
            this.states[0][0] = new FallItemStateRow(1, 0);
